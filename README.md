...
### 9. Pivotagem de Arquitetura: Segmentação Semântica Multimodal (Fase 2.1)
* **Abandono do Determinismo Ótico:** Remoção das rotinas rígidas do OpenCV para a localização de marcadores impressos, mitigando o risco de falhas por rasuras ou interferências gráficas dos alunos na prova.
* **Visão Computacional via IA Generativa:** Adoção do modelo multimodal Gemini 3.5 Flash para realizar a **Segmentação Semântica**. O fluxo foi unificado: a IA processa a fotografia integral, diferencia cognitivamente rabiscos da resolução matemática, fornece a análise pedagógica e retorna as coordenadas espaciais normalizadas `[ymin, xmin, ymax, xmax]` da área de caligrafia.
* **Recorte Matemático Dinâmico:** O Python (`corretor.py`) atua como um motor de conversão geométrica geométrica, traduzindo as coordenadas da nuvem em um recorte de imagem exato para visualização e auditoria do professor.

### 10. Evolução para Sistema Multi-Tenant e Armazenamento (Fase 3)
* **Persistência de Dados e Segurança:** Introdução de um banco de dados relacional (`SQLite3`) para substituir variáveis efêmeras na memória. Implementação de um módulo de segurança com criptografia de senhas (função de hash `SHA-256` via biblioteca `hashlib`).
* **Arquitetura de Dupla Porta (Multi-Tenant):** Implementação de um modelo hierárquico de acesso. A primeira camada exige uma *Senha Global* para isolar a infraestrutura de *bots*. A segunda camada gerencia a sessão (`st.session_state`) com login individual, garantindo o isolamento completo dos dados pedagógicos de cada professor.
* **Seletor de Contexto (Context Switcher):** Otimização de usabilidade (UX) mapeando a realidade docente (múltiplos vínculos empregatícios). O sistema permite alternar o "contexto ativo" entre escolas, filtrando o histórico e ajustando automaticamente os cabeçalhos das avaliações geradas.
* **Construtor Dinâmico de Avaliações (Motor LaTeX Nativo):** Refatoração da rotina estática `fabri_ava.py` para um pipeline dinâmico sem dependência de pacotes LaTeX externos (`.sty`). O motor agora compila os metadados através da classe universal `exam`.
* **Engenharia de Frontend (Callbacks e Vetores):** Utilização intensiva do padrão *Callback* do Streamlit (`on_click`) para manipular um vetor de estado em memória, permitindo ao professor adicionar, excluir e reordenar questões (↑ e ↓) sem perda de dados (refresh) na interface.
* **Arquivo Pedagógico em Nuvem (Gestão de BLOBs):** Solução do problema de servidores efêmeros transformando os PDFs compilados em dados binários (*Binary Large Objects*). As provas e os pareceres da IA ficam imutáveis no banco de dados para *download* assíncrono, automatizando o ciclo de vida dos arquivos com rotinas de descarte seguro de lixo (*Garbage Collection* das imagens temporárias).
