...
st.write("Bem-vindo! Escolha uma das opções abaixo para gerenciar suas avaliações de Física.")
...
with aba_fabricar:
    ...
    if st.button("Salvar e Gerar PDF para Impressão"):
        if colegio == "Selecione o colégio..." or not turma:
            ...
            with st.spinner("Compilando código LaTeX nos bastidores..."):
                ...
                if os.path.exists("prova_gerada_app.pdf"):
                    ...
                    st.error("Ocorreu um erro na compilação do arquivo LaTeX.")
...
with aba_corrigir:
    ...
    st.write("Envie a foto da folha de respostas tirada pelo smartphone.")
    ...
    if foto_prova is not None:
        ...
        if st.button("Executar Algoritmo de Recorte (Visão Computacional)"):
            with st.spinner("Buscando âncoras fiduciais na imagem..."):
                caminho_recorte = testar_recorte_contexto("upload_temp.jpg", "icone_equacao.jpg")
                
                if caminho_recorte:
                    st.success("Limites de cálculo isolados com perfeição.")
                    st.image(caminho_recorte, caption="Bloco enviado para a IA (Extraído)")
                else:
                    st.error("Não foi possível localizar o par de marcadores da equação nesta imagem.")